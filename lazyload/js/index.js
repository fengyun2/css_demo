~ function(window, undefined) {
    ...
    Lazyload.prototype._detectElementIfInScreen = function() {
        ...
        for (var i = 0, len = this.elements.length; i < len; i++) {
            ...
            if (!this._pause && (rect.top >= this.distance && rect.left >= this.distance || rect.top < 0 && (rect.top + rect.height) >= this.distance || rect.left < 0 && (rect.left + rect.width >= this.distance)) && rect.top <= H && rect.left <= W) {
                this.loadItem(ele);
    ...
    function $(query) {
        ...
        if (document.querySelectorAll) {
            ...
            firstStyleSheet.addRule(query, 'baby');
